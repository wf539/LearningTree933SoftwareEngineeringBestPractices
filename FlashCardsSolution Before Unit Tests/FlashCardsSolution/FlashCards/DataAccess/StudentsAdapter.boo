...
"""Description of StudentsAdapter"""
	...
	def GetStudentsForTeacher(teacher as Teacher) as List:
		...
		reader = DatabaseHelper().ExecuteSelect("Select * FROM Students WHERE TeacherID = @TeacherID", params)
	...
	def GetScoresForStudent(student as Student) as List:
		...
		reader = DatabaseHelper().ExecuteSelect("Select * FROM Scores WHERE StudentID = @StudentID", params)
